...
export async function createTiendanubeOrder(input) {
  ...
  if (!draftResponse.ok) {
    ...
    throw orderError(
      apiErrorMessage(draft, "Tiendanube rechazó el pedido."),
      502,
      "draft_order_failed",
    );
  }
  ...
  if (!confirmResponse.ok) {
    throw orderError(
      apiErrorMessage(order, "No se pudo confirmar el pedido."),
      502,
...
export function prepareDraftOrder(input, products, options = {}) {
  ...
  addIfPresent(address, "floor", customer.floor);
  addIfPresent(address, "locality", customer.locality);
  ...
  addIfPresent(result.payload, "cpf_cnpj", customer.document);
  return result;
}
...
function addIfPresent(target, field, value) {
